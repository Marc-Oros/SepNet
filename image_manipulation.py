...
def separate_objects(img, caption_words, synonyms, annotations, val_annotations, img_id):
    view_results = False
    min_bbox_size = 4
    ...
    annIds = train_annotations.getAnnIds(img_id)
    anns = train_annotations.loadAnns(annIds)
    if len(annIds) > 0:
        imgInfo = train_annotations.loadImgs(img_id)
    if len(annIds) == 0:
        annIds = val_annotations.getAnnIds(img_id)
        anns = val_annotations.loadAnns(annIds)
        if len(annIds) > 0:
            imgInfo = val_annotations.loadImgs(img_id)
    ...
    for annotation in anns:
        catinfo = train_annotations.loadCats(annotation['category_id'])[0]
